...
class TestRunPipeline(unittest.TestCase):
    def test_run_pipeline_calls_parts_pipeline_when_enabled(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            model_path = Path(tmpdir) / "source" / "circuit-model.source.json"
            ...
            fake_placement_result = {
                "ok": True,
                "plan_file": str(Path(tmpdir) / "proj-1" / "build" / "placement-plan.json"),
                "report_file": str(Path(tmpdir) / "proj-1" / "build" / "placement-plan.md"),
                "plan": {"summary": {"region_count": 1, "placed_count": 1, "unassigned_count": 0}},
            }
        ...
        self.assertEqual(summary["files"]["placement_plan"], fake_placement_result["plan_file"])
    ...
    def test_run_pipeline_skips_parts_when_disabled(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            fake_placement_result = {
                ...
                "plan_file": str(Path(tmpdir) / "proj-1" / "build" / "placement-plan.json"),
                "report_file": str(Path(tmpdir) / "proj-1" / "build" / "placement-plan.md"),
                "plan": {"summary": {"region_count": 0, "placed_count": 0, "unassigned_count": 0}},
            }
