...
function main(): void {
  if (!testUuid(extensionConfig.uuid)) {
    ...
    // Remove the synthetic default export produced by JSON import handling.
    // @ts-expect-error The JSON module type includes a default export helper.
    delete newExtensionConfig.default;
    newExtensionConfig.uuid = fixUuid(extensionConfig.uuid);
    fs.writeJsonSync(path.join(__dirname, '../extension.json'), newExtensionConfig, {
      spaces: 2,
      EOL: '\n',
      encoding: 'utf-8',
    });
  ...
  const filepathListWithoutFilter = fs.readdirSync(path.join(__dirname, '../'), {
    encoding: 'utf-8',
    recursive: true,
  });
