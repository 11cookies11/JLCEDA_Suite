...
本文把当前 H618 AgentBoard V1 从“KiCad 可生成、ERC 归零”推进到“可评审、可打样前检查”的工作拆成明确阶段。
...
- 确认 UART0 调试口、电源、地和测试点可在 bring-up 阶段直接使用。
- 将启动路径风险写入 docs 和 `risks`。

完成标准：
- MicroSD、SPI NOR、UART0、RESET、FEL 均有明确网络和默认状态。
- 不再依赖聚合网表达启动关键路径。

## 阶段 3：LPDDR4 参考设计对齐

目标：从当前 DDR 骨架进入真实可评审 DDR 拓扑。

任务：
- 按成熟 H618/H616 参考设计补全 DDR pinmap。
...
2. H618 最小启动闭环。
3. LPDDR4 参考设计对齐。
4. RGMII、USB Hub、HDMI、USB-C 外设闭环。
5. 可打样前审查包。
