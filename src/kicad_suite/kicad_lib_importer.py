...
from .part_selector import SelectedPart, classify_package_risk, package_risk_note
...
def _build_risk_report(parts: list[SelectedPart]) -> str:
    ...
    if high_risk:
        for part in high_risk:
            ...
            lines.append(f"**{part.requirement_id}** {part.mpn} {part.package} {part.lcsc_id}")
            lines.append(f"- {note}")
            lines.append(f"- KiCad → EasyEDA Pro 转换可能失败")
            lines.append(f"- 建议在 EasyEDA Pro 中用 LCSC 编号重新放置")
            lines.append("")
    else:
        lines.append("(none)")
        lines.append("")
