...
from .compile_kicad_execution_plan import (
    ...
    KiCadTarget,
    _auto_position,
    _compute_block_layout,
    _estimate_symbol_size,
    _resolve_block,
...
from .schema_versions import KICAD_EXECUTION_PLAN_SCHEMA_VERSION
...
def ir_to_kicad(ir: dict[str, Any]) -> KiCadExecutionPlan:
    ...
    from .compile_kicad_execution_plan import _validate_symbol_libraries
...
def _resolve_position(
    *,
    ...
    row = index // columns
    return KiCadPoint(
        x=_snap(origin_x + col * pitch_x),
        y=_snap(origin_y - row * pitch_y),
