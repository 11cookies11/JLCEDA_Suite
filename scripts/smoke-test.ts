...
function installMockEda(): void {
  ...
  (globalThis as { fetch?: typeof fetch; __lastUpdateFetchRequest?: typeof lastFetchRequest }).fetch = async (input, init) => {
    ...
    return new Response(JSON.stringify({
      tag_name: 'v0.1.25',
      html_url: 'https://github.com/11cookies11/JLCEDA_Suite/releases/tag/v0.1.25',
      assets: [
        {
          name: 'jlceda-suite_v0.1.25.eext',
          browser_download_url: 'https://github.com/11cookies11/JLCEDA_Suite/releases/download/v0.1.25/jlceda-suite_v0.1.25.eext',
        },
      ],
    }), {
      status: 200,
    ...
    },
    {
      ...
      verify: (response) => {
        ...
        if (response.status === 'success') {
          ...
          assert(data.latestVersion === 'v0.1.25', 'update check should surface the mocked latest version');
          assert(data.latestDownloadUrl?.includes('v0.1.25'), 'update check should surface the mocked download url');
        }
      },
    },
    {
      ...
      verify: (response) => {
        ...
        if (response.status === 'success') {
          ...
          assert(data.latestVersion === 'v0.1.25', 'update status should expose the mocked latest version');
        }
      },
    },
    {
    ...
    },
    {
      ...
      verify: (response) => {
        ...
        if (response.status === 'success') {
          ...
          assert(data.latestVersion === 'v0.1.25', 'private update check should surface the mocked latest version');
        }
      },
    },
    {
