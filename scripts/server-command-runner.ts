...
async function loadPlan(): Promise<RunnerPlan> {
  const inlinePlan = getEnv('BRIDGE_COMMANDS_JSON');
  if (inlinePlan) {
    return JSON.parse(inlinePlan) as RunnerPlan;
  }

  const planFile = getEnv('BRIDGE_COMMANDS_FILE');
  if (planFile) {
    return JSON.parse(await fs.readFile(planFile, 'utf8')) as RunnerPlan;
  }
  ...
  if (!process.stdin.isTTY) {
    ...
    if (raw) {
      return JSON.parse(raw) as RunnerPlan;
    }
  }
